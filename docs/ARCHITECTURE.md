...
- **Scheduler:** next fetch comes from the `Expires` header; conditional requests with `ETag` / `If-None-Match`; interactive requests jump ahead of bulk syncs.
- **Cache:** eve-esi-client's response cache is `core.esi_cache` in Postgres (`tether_esi::cache::PgCache`): shared by every worker, kept across restarts, keyed by URL and the token's `sub` (empty for public requests), pruned hourly by expiry and a row cap. Only the host's own unauthenticated requests use it. Calls carrying a character's token (plugin calls, Corp Stats) never do: the library keys its cache by the token it sees on the request, and these carry theirs as a client default header, so they go through `EsiInner::without_cache()` and ESI checks the character's scopes and roles every time. A response ESI marks `private` or `no-store` is never stored without a principal.
- **Budgets:** eve-esi-client tracks ESI's error-limit and rate-limit budgets and slows down before hitting them; the host reads them from its accessors (`error_budget()`, `rate_budgets()`), never from response headers, which cached responses don't carry, and counts what came back (fresh, cached, revalidated, errors) for the dashboard; bulk work backs off earlier than interactive requests; plugin calls share the same limiters; per-plugin shares, so one misbehaving plugin is throttled and flagged instead of getting the whole instance blocked.
...
- `esi_cache`: cached ESI responses (status, headers, body out of line, ETag, expiry), host-only; see the ESI layer.
...
- Sudo mode (`crates/web/src/sudo.rs`), as GitHub's: owner-only and especially sensitive actions need an EVE login with the account's main in the last 15 minutes. The session records when that last happened (`core.sessions.reauthenticated_at`): a plain login with the main sets it, and so does re-authenticating; Add Character, offers, Change Main and a login that makes a character the main of a main-less account prove only some character, so their new session keeps the old one's time. (The main-less case is defence in depth: once that character is the main, logging in with it again confirms. What stops a stolen, stale session planting a main is that it can't link a character or delete the main's token without sudo.) Otherwise the action is refused before anything changes, and the browser goes to "Confirm it's you" (`/reauthenticate`), which names the action; its button starts an EVE login (purpose `reauth`) that must be the signed-in account's main with its recorded owner hash (nothing is linked, moved or stored), audited as `session.reauth` with the action. A refused gated action is audited too (`session.sudo_required`, with the action and path), so a stale session probing is visible in the audit log. Taking sensitive permissions away is gated like giving them: revoking a grant, removing someone from (or deleting) a group that grants one, deleting an Auto Groups config whose groups do, and blacklisting an account that holds one (as deactivating). Afterwards the browser is back on the page it posted from (its same-site Referer, else the Dashboard), where the admin submits again: nothing is replayed, and a POST never is. htmx requests are sent there with `HX-Redirect`; the JSON API answers 403. The gated actions, each checked where the action happens so pages and the API share it:
