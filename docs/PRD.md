...
| N6 | Opsec | Admins sign in like everyone else: EVE SSO on the public domain, with no separate admin login, listener or private network. Admin pages and API endpoints are gated by permissions, checked on the server for every request. Owner-only and sensitive actions (sensitive permission grants, app install, upgrade, rollback, uninstall, key re-pin and secrets, account deactivation, access tokens, setup and Discord settings, Restricted groups, letting people into groups that grant sensitive permissions, and Change Main, Add Character and deleting the main's token for accounts holding such powers) also need an EVE login with the account's main in the last 15 minutes (sudo mode, as GitHub's; see docs/ARCHITECTURE.md) |
...
- [x] `eve-esi-client` follow-ups (Jay): re-export its oauth2 types and allow overriding SSO URLs (so `EveSso` can be tested against wiremock), and a pluggable cache hook plus public budget accessors, so the host can back ESI responses with a shared Postgres cache that survives restarts: done in 0.6.0, and Tether uses all of them (`core.esi_cache`)
