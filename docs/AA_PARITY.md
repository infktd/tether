...
| Change Main | switch the main to a character with a valid token, or log in with one through SSO (added, or taken from another account); state re-evaluated | the same: **Change Main** on a character with a working token (refreshed first, as AA's `require_valid`), or **Change Main with EVE login** for any character; audited; state re-evaluated (see Behaviour) | done |
...
| Structure Timers (timerboard: structure, timer type, objective, system, planet/moon, EVE time, details, important, corporation timers; `timer_view`, `timer_management`) | Structure Timers plugin (`plugins/structure-timers`): Upcoming and Past with countdowns, Create Timer from the EVE time or the time left, Edit Timer and delete; corporation timers seen by the creator's corporation only; Structures' timers listed with them as automatic, read-only timers marked with the app's name, as aa-structures feeds the timerboard | done; stricter than AA: a corporation timer can't be edited or deleted from outside its corporation. Countdowns are drawn when the page loads (plugin pages have no live updates) |
| Ship Replacement (SRP: SRP fleets with name, doctrine, FC, time, AAR and SRP code; Request SRP by killboard link; approve, reject, update payout, mark completed, remove; Total ISK Cost; `access_srp`, `add_srpfleetmain`, `manage_srp`, `change_srpuserrequest`) | Ship Replacement plugin (`plugins/ship-replacement`): **SRP Fleets** (Open or Completed, request links, pending and Total ISK Cost for staff), **Add SRP Fleet**, **Request SRP** by zKillboard or ESI killmail link with the pilot's character and additional info, **My SRP Requests**, **SRP Fleet Data** (requests, totals, Mark Completed or Incomplete, Mark Approved Paid, Remove Fleet), and a page per request to approve, reject, update the payout and mark it paid, with comments; totals per fleet and overall (losses, Total ISK Cost, paid, outstanding) | done; the loss is read from ESI's public killmail endpoint and must be the chosen character's, one of the pilot's own (AA checks the killboard's victim too), each loss once; its value from zKillboard (`zkillboard.com`, approved by Jay, fetched once and kept); approving without a set payout pays zKillboard's value, as AA; paid requests are settled (no more changes). Stricter than AA: nobody decides, prices or pays their own request; changing an approved payout sends it back to pending; a loss stays claimed when its fleet is removed; links that don't check out are limited per pilot; a loss more than a day from the fleet time is flagged for reviewers |
...
| App | Tether | Status |
| --- | --- | --- |
| Member Audit (My Characters, Character Sheet, Character Finder, Reports: User Compliance, Corporation Compliance, Skill Sets; Compliance Groups; sharing) | compliance, Compliance Groups and Register Character are core; the Member Audit plugin (`plugins/member-audit`) has My Characters (with combined totals and queues for multiboxers), the Character Sheet (skills, queue, assets, wallet journal, clones, implants, location, ship), Character Finder, Skill Sets and the Skill Sets report; Secure Groups filters (a skill at a level, a skill set, an item in the assets), as aa-securegroups' Member Audit filters | done, except mail, contacts, contracts, loyalty, planets and character sharing (more scopes every Member would need; on request) |
...
| Structures (Add Structure Owner; structure list with fuel, services, state, reinforce hour; starbases with fuel, state and reinforcement; customs offices (Orbitals) with taxes and access, and the public customs office list; Orbital Skyhooks; Metenox moon drills; structure fittings and quantum cores; tags (generated and custom) and the tag filter; low fuel and reinforced filters; notifications to Discord webhooks per owner; fuel alerts; timers to the timerboard; `basic_access`, `view_corporation_structures`, `view_alliance_structures`, `view_all_structures`, `view_structure_fit`) | Structures plugin (`plugins/structures`): owners are approved data sources with the Station Manager role (AA's Add Structure Owner; starbases, customs offices, skyhooks and fittings need the Director role, as there, and an owner without it is backed off for those reads alone); the list (owner, name, type, system with security, region, fuel expiry and time left, services, state and state timer, reinforce hour, core, tags) with Low fuel, Reinforced, Timers, Starbases (moon, fuel, strontium, state, reinforced until), Orbitals (customs offices with planet, reinforcement exit window and taxes; skyhooks at their planet), Tags and Owners tabs, a page per owner and per structure (general, fuel, a customs office's taxes and access, a starbase's fuel bay, the fitting by slot with fighters, fuel bay, quantum core and moon material bay for `view_structure_fit`, tags); starbase fuel from its fuel bay at 40, 20 or 10 blocks an hour by size, a quarter less in the owner's alliance's sov (as aa-structures); a Metenox's fuel is the earlier of its blocks (ESI's) and its magmatic gas at 200 an hour (read from its fuel bay; aa-structures shows only the blocks); the owners' notifications (Upwell attacks, reinforcements, fuel, services, power, anchoring, Metenox reagents, starbase attacks and resources, customs offices attacked and reinforced, skyhooks deployed, online, attacked, reinforced and destroyed, moon extractions) and aa-structures' own "starbase reinforced" relayed once each, within a day, to the channel a manager picks per kind, optionally mentioning Members on attacks; per owner (aa-structures' webhooks per owner) its own channel per kind, or none, and its own mention setting, the settings' channels being the defaults; Tether's low-fuel alerts at up to 5 thresholds for Upwell structures (Metenox gas included) and starbases; tags as aa-structures' (name, description, style, order, default: put on structures first seen after), managers make them and set them on a structure's page, generated highsec, lowsec, nullsec, w_space and sov tags kept by the sync, the list filtered by any of some tags, and optionally only structures with a default tag (STRUCTURES_DEFAULT_TAGS_FILTER_ENABLED); customs offices public per owner (`are_pocos_public`) on a list for everyone with `basic_access`, with the viewer's access and tax by corporation and alliance; AA's permission names plus `manage` (channels, thresholds, routing, tags, retrying an owner); timers (armor, hull, final for starbases, customs offices and skyhooks, anchoring, unanchoring) fed to Structure Timers after every sync, friendly, corporation-only when "Timers are corporation-only" is on (STRUCTURES_TIMERS_ARE_CORP_RESTRICTED, off by default) | done. Plugins can't reach Discord webhooks: the bot posts to channels an admin assigned, which stand in for them, routed by four kinds (attacks, fuel and services, state, moon) rather than aa-structures' per-type lists, and per owner, not per structure. What ESI can't do: Orbital Skyhooks are only in the corporation's assets, with no name, state, reagents or timers (their notifications give those; they burn no fuel), and their planet is the nearest one to where the assets put them; a starbase's fuel is as of the last read (ESI caches it an hour) and faction towers burn as their size, as in aa-structures; a customs office's tax by standing can't be worked out for a viewer (Tether has no standings), so the public list shows the neutral rate as "By standing". Not taken: ammunition in the ammo hold (its `Cargo` flag is also every corporation ship's cargo, which the host doesn't pass on), jump gates' liquid ozone alerts, refuelled notifications, alliance-level (sov, war) notifications and the NPC attack filter (the host passes on only structure notifications). ESI is read at its cache times (notifications 10 minutes, the rest an hour), and an owner ESI refuses (a lost role or token) is left alone for an hour, doubling to a day, per read, until a manager retries it |
...
| Secure Groups (Smart Groups, filters, grace) | **Secure Groups** (Admin → Groups → a group): smart groups kept by filters (state; main's or any character's corporation or alliance; main's age; other groups, any or all; compliant; each reversible), auto join or requests, a grace period with notifications, an hourly sweep, the group's Audit Log; app filters: Member Audit's skill, skill set and asset, and Fleet Activity Tracking's FATs in the last days | done. An app filter with no fresh report leaves its group alone, and a reversed one passes only when every character was reported |
| AA SRP, AFAT | SRP is the Ship Replacement plugin (above); AFAT's additions (alliance stats, fleet types, expiring links, ESI-tracked fleets, logs) are in the Fleet Activity Tracking plugin | SRP: done; AFAT: done |
...
- **Change Main** (AA's `main_character_change`): pick one of the account's characters with a valid token, or log in with any character through EVE SSO, which adds it to the account (moving it from another account, as above) and makes it the main. As AA's `require_valid`, a token whose access token has expired is refreshed first, so a sale or revocation since the last ownership check counts at once: a sold character leaves the account, a revoked one can only become main by logging in with it. The change is audited and the state follows the main. Deliberately different: if SSO can't be reached, the stored token state (kept by the ownership check) decides, so an outage doesn't block it; and characters without a working token show a login button instead of AA's shorter token list.
...
3. **Other AA apps.** ~~The pilot alliance uses none beyond Member Audit and Moon Mining for now, so Fleet Activity Tracking, Structure Timers, Structures, SRP and HR Applications stay off the plan until asked.~~ Superseded by 4.
4. **Feature parity is 1:1 with AA core** (2026-09-25, Jay): AA's bundled apps (Fleet Activity Tracking, Structure Timers, Ship Replacement, HR Applications; Fleet Operations is F22) are built as first-party plugins (bundled into the image, so every deployment has them: 2026-09-26), and Menu customization moves into milestone 2. Services other than Discord (Mumble, TeamSpeak 3, Openfire, the forums) come after launch: Discord is the one alliances ask for. Community apps stay as listed.
