...
| Structures (Add Structure Owner; structure list with fuel, services, state, reinforce hour; low fuel and reinforced filters; notifications to Discord webhooks; fuel alerts; timers to the timerboard; `basic_access`, `view_corporation_structures`, `view_alliance_structures`, `view_all_structures`) | Structures plugin (`plugins/structures`): owners are approved data sources with the Station Manager role (AA's Add Structure Owner); the list (owner, name, type, system with security, region, fuel expiry and time left, services online or offline, state and state timer, reinforce hour) with Low fuel, Reinforced, Timers and Owners tabs and a page per owner; the owners' structure notifications (under attack, lost shields or armor, destroyed, fuel alert, services offline, low and high power, online, anchoring, unanchoring, moon extractions) relayed once each, within a day, to the channel a manager picks per kind, optionally mentioning Members on attacks (every owner's, whatever the view permissions, as aa-structures' webhooks; the settings page says so), only for structures seen in the owner's corporation; Tether's own low-fuel alerts at up to 5 thresholds; AA's permission names plus `manage` (channels, thresholds, retrying an owner); timers (armor, hull, anchoring, unanchoring) fed to Structure Timers after every sync, friendly, corporation-only when "Timers are corporation-only" is on (STRUCTURES_TIMERS_ARE_CORP_RESTRICTED, off by default) | partial: the bot posts to assigned channels (no webhooks). Not yet: starbases (POS), customs offices, Skyhooks and Metenox fuel, structure fittings, and aa-structures' tags and per-owner webhooks. ESI is read at its cache times (notifications 10 minutes, structures an hour), and an owner ESI refuses (a lost role or token) is left alone for an hour, doubling to a day, until a manager retries it |
