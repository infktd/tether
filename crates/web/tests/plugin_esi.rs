...
async fn user_scopes_need_the_characters_consent(db: PgPool) {
    ...
    for uri in [
        "/profile/plugins/nmu.esi/consent",
    ...
    assert_eq!(out, "err Error::NotConsented");

    // The profile offers it; granting asks EVE for the plugin's scope.
    let profile = page(&h, "/profile", &owner).await.body;
    assert!(
        profile.contains("ESI probe") && profile.contains(SKILLS),
        "{profile}"
    );
    ...
    let consented = probe(&h, "consented", &[]).await;
    assert!(
        consented.contains("Chribba") && consented.contains(SKILLS),
        "{consented}"
    );
    ...
    let res = send(
        &h.app,
        form(
            &format!("/profile/plugins/nmu.esi/consent/{CHRIBBA}/revoke"),
            "",
            &owner,
        ),
    )
    ...
    // Every call is in the access log; consent changes are in the audit log.
    let log = access_log(&h.db).await;
    assert!(
        log.contains(&("character-skills".to_owned(), "ok".to_owned())),
        "{log:?}"
    );
    assert!(log.contains(&("character-skills".to_owned(), "not consented".to_owned())));
    let audited: Vec<String> = sqlx::query_scalar(
        "SELECT action FROM core.audit_log WHERE action LIKE 'plugin.consent%' ORDER BY id",
    )
    .fetch_all(&h.db)
    .await
    .unwrap();
    assert_eq!(
        audited,
        ["plugin.consent_granted", "plugin.consent_revoked"]
    );
...
#[sqlx::test(migrator = "tether_db::MIGRATOR")]
async fn a_plain_login_keeps_the_scopes_a_plugin_was_granted(db: PgPool) {
    let h = harness(db, true).await;
    let owner = log_in_owner(&h, "196379789:Chribba").await;
    install(&h, &owner).await;
    mount_esi(&h).await;
    ...
    .await;
    // Logging in again (no scopes) mustn't replace the richer token.
    log_in_as(&h, "196379789:Chribba", Some(&owner)).await;
    let out = esi(&h, "character-skills", ("character", CHRIBBA)).await;
