...
cargo fmt --all
cargo clippy -p hello-plugin -p moon-mining -p member-audit -p fleet-activity-tracking -p structure-timers -p hr-applications -p tether-plugin-sdk --target wasm32-wasip2 -- -D warnings   # plus the tether-plugins-test-guest* crates
